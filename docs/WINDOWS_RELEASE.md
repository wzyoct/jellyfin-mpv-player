...
便携配置、登录状态和缓存位于同目录的 `data/`。移动整个 `release/` 文件夹时请保留 `resources/`、`locales/` 和所有 DLL/PAK 运行文件。
...
`data/` 永远不参与发布文件覆盖、归档和 SHA256 计算。`历史版本` 保存旧安装器和旧发布说明，`构建内部文件` 保存原始 `win-unpacked` 构建和旧运行文件；普通用户不需要打开这两个目录。
