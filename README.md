...
字幕默认优先级为：外挂简中、外挂其他中文、服务端默认外挂、第一条外挂；只有没有外挂时才按内嵌简中、内嵌其他中文、服务端默认内嵌选择。`DeliveryMethod=Encode` 表示已烧录进画面的字幕，不作为可选轨道，也不会映射到 MPV。服务器外挂字幕按当前媒体源生成标准 Jellyfin 字幕接口，再经本地鉴权网关加载；STRM 播放会自动加载媒体库中的外挂字幕，无需手动选择。Windows 能直接访问媒体源目录时，也会优先尝试挂载同目录同名字幕或目录中唯一的字幕文件。详情页仍保留手动选择本地 ASS、SSA、SRT、VTT、SMI 或 SUB 文件的能力。
