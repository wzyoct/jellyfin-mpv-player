...
import { EmbyClient, normalizeServerUrl } from './emby'
...
describe('normalizeServerUrl', () => {
  it('adds the Emby API path and removes web/query fragments', () => {
    expect(normalizeServerUrl('  media.example.test/web/index.html?foo=bar#section ')).toBe('http://media.example.test/emby')
  ...
  it('rejects an empty address', () => {
    expect(() => normalizeServerUrl('  ')).toThrow('请输入 Emby 服务器地址')
  })
})
...
describe('EmbyClient', () => {
  ...
  it('sends authorization and preserves caller headers', async () => {
    ...
    const client = new EmbyClient('https://media.example.test', 'secret-token', 'user-1', 'device-1')
  ...
  it('authenticates without sending a token and handles empty responses', async () => {
    ...
    await expect(EmbyClient.authenticate('media.example.test', 'mickey', 'password')).resolves.toEqual({
      AccessToken: 'token',
    ...
    await expect(new EmbyClient('media.example.test', 'token', 'u1').request('/Sessions/Playing/Stopped', { method: 'POST' })).resolves.toBeUndefined()
  })
  ...
  it('surfaces HTTP, network and JSON parsing failures', async () => {
    ...
    await expect(new EmbyClient('media.example.test', 'token', 'u1').request('/System/Info')).rejects.toThrow(/^Emby 请求失败（502）：x{180}$/)

    fetchMock.mockRejectedValueOnce(new Error('offline'))
    await expect(new EmbyClient('media.example.test', 'token', 'u1').request('/System/Info')).rejects.toThrow('offline')

    fetchMock.mockResolvedValueOnce(new Response('{not-json', { status: 200 }))
    await expect(new EmbyClient('media.example.test', 'token', 'u1').request('/System/Info')).rejects.toThrow()
  })

  it('builds query requests and paginates series episodes', async () => {
    const client = new EmbyClient('media.example.test', 'token', 'user/1')
  ...
  it('calls views, recommendations, item and playback endpoints', async () => {
    const client = new EmbyClient('media.example.test', 'token', 'u1')
  ...
  it('reads images and builds stream and subtitle URLs', async () => {
    ...
    const direct = client.buildStreamUrl('item/1', {
      Id: 'source-1',
      DirectStreamUrl: 'https://cdn.example.test/file.mp4?existing=1',
  ...
  it('rejects binary responses with a useful status', async () => {
    ...
    await expect(new EmbyClient('media.example.test', 'token', 'u1').requestBinary('/missing')).rejects.toThrow('图片请求失败（404）')
  })
